...
): groupedEvent is InMemoryGroupedEvent => groupedEvent.eventStorageAdapter instanceof InMemoryStorageAdapter;
...
export class InMemoryStorageAdapter implements StorageAdapter {
  ...
  constructor({ initialEvents = [] }: { initialEvents?: EventDetail[] } = {}) {
    ...
    this.pushEvent = async (event, context) => new Promise((resolve) => {
      const timestamp = new Date().toISOString();
      resolve(this.pushEventSync({ timestamp, ...event }, context));
    });

    this.pushEventGroup = async (...groupedEventsInput) => new Promise((resolve) => {
      const { groupedEvents, timestamp = new Date().toISOString() } = parseGroupedEvents(...groupedEventsInput);
        ...
        try {
          ...
          // Revert it in reversed order
            ...
            .forEach((groupedEventToRevert) => {
              ...
              const revertedEvent = eventToRevertStorageAdapter.eventStore[aggregateId]?.pop();
    ...
    this.getLastSnapshot = () => new Promise((resolve) => resolve({ snapshot: undefined }));
    this.listSnapshots = () => new Promise((resolve) => resolve({ snapshots: [] }));
  }
}
